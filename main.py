...
def planet_generator(n, w, h):
    ...
    terrain = [small_shop_up, small_shop_down, middle_shop1_down, middle_shop2_down, middle_shop1_up, middle_shop2_up,
               big_shop_down, big_shop_up, mystery_shop, place_work] + obstacles
            ...
            for i1 in range(i - 2, i + len(obj) + 2):
                ...
                for i1 in range(i, i + len(obj)):
                    for j1 in range(j, j + len(obj[0])):
                        ...
                        elif field[i1 % h][j1 % h] == 'w':
                            merchant = {'name': generate_name(randint(5, 6)).capitalize(),
                                        'home planet': random.choice(PLANET_NAMES),
                                        'image num': randint(0, len(merchants_images) - 1),
                                        'change': 'WORK',
                                        'last trade': None}
                            MERCHANTS.append(merchant)
                            field[i1 % h][j1 % h] = f'w{len(MERCHANTS) - 1}'
...
class Tile(pygame.sprite.Sprite):
    def __init__(self, tile_type, pos_x, pos_y, level_type='GREEN'):
        ...
        if tile_type == 's':
            ...
            self.image = pygame.transform.scale(mystery_merchants_images[0],
                                                (round(tile_width * 0.94), tile_height * 2))
            self.rect = self.image.get_rect()
            self.rect.center = (round(tile_width * (pos_x + 0.5)), round(tile_height * (pos_y + 0.5)))
        elif tile_type[0] == 'u':
...
def show_action(player):
    ...
    if tile.type == 'l':
        ...
        send_message('Подработка! Бесплатно даём деньги! Жми, Enter.')
...
def trade_game(screen, merchant, player):
    ...
    if merchant['change'] not in ['UPGRADE', 'WORK']:
        ...
        k, w = draw_text((WIDTH - window_w) // 2 + 10 + move_right, k,
                         ...
                         (255, 255, 255), screen, 50, line_size=20, fon_color=(128, 128, 128))
        button = [(WIDTH - window_w) // 2 + 90 + move_right, k]
        k, w = draw_text((WIDTH - window_w) // 2 + 10 + move_right, k, 'Работать!', (255, 255, 255), screen, 30,
                  width=window_w - 180, fon_color=(128, 128, 128))
        button.append(w)
        button.append(k - button[1])
        buttons.append(button)
